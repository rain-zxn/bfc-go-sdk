...
const BfcAddressLen = 32

type AccountAddress [BfcAddressLen]uint8

func NewAccountAddressHex(str string) (*AccountAddress, error) {
	if strings.HasPrefix(str, "0x") || strings.HasPrefix(str, "0X") {
		str = str[2:]
	} else if strings.HasPrefix(str, "bfc") || strings.HasPrefix(str, "BFC") {
		str = str[3 : len(str)-4]
	}
	if len(str)%2 != 0 {
		str = "0" + str
	...
	if len(data) > BfcAddressLen {
		return nil, errors.New("the len is invalid")
	}
	var accountAddress AccountAddress
	copy(accountAddress[BfcAddressLen-len(data):], data[:])
